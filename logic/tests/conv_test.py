...
class ConvTest(unittest.TestCase):
    ...
    def testTryConv(self):
        ...
        t = lf(x)
        self.assertEqual(cv(thy, t), Thm.beta_conv(t))
        self.assertEqual(cv(thy, x), Thm.reflexive(x))
    ...
    def testRewrConv(self):
        ...
        self.assertEqual(cv1(thy, one), eq_th)
        self.assertEqual(thy.check_proof(prf), eq_th)
        self.assertRaises(ConvException, cv1, thy, zero)
        ...
        eq1 = eq(f(one), g(one))
        self.assertEqual(cv2(thy, f(zero)), eq0)
        self.assertEqual(cv2(thy, f(one)), eq1)
    ...
    def testRewrConvWithAssum(self):
        ...
        self.assertEqual(cv(thy, f(x)), res)
        prf = Proof()
    ...
    def testTopBetaConvLarge(self):
        ...
        for i in range(8):
            ...
            res = f(res, res)
        prf = cv.get_proof_term(thy, t).export()
        self.assertEqual(cv(thy, t), eq(t, res))
