...
#global var file_data for storing opened json file; for deleting and adding function;
global file_data
file_data = dict()
...
def save_edit_file():
    data = json.loads(request.get_data().decode("utf-8"))
    save_edit(data['name'], data['data'], data['n'], data['ty'], [v for v in file_data.values()])
...
def delete_file():
    ...
    id = data['id']
    del file_data[id]
    delete(data['name'], [d for d in file_data.values()])
