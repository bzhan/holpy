...
def apply_rule(rule, facts, *, lines=None, record=False, circles=None, ruleset=None, hyps=None):
    ...
    for inst in insts:  # An inst represents one applying result.
        if rule.concl.args[0].islower():
            cond = get_cond(divisors_for_apply[rule.concl.pred_name]/2)
            concl_args = []
            for i in rule.concl.args:
                concl_args.extend((inst[i][0], inst[i][1]))
        else:
            cond = get_cond(divisors_for_apply[rule.concl.pred_name])
