...
class ProofReconstruction:
    ...
    def validate_step(self, macro_name, args, prevs=None, is_eval=True, ctx=None, step_args=None):
        if ctx is not None:
            args = args + (self.ctx,)
    ...
    def validate(self, is_eval=True):
        with alive_bar(len(self.steps)) as bar:
            for step in self.steps:
                # print("step", step.id)
                if isinstance(step, command.Assume):
                    ...
                    elif rule_name == "la_generic":
                        self.pts[step.id] = self.validate_step("verit_la_generic", step.cl, premises, is_eval=is_eval, step_args=step.args)
                    # elif rule_name == "lia_generic":
                    #     self.pts[step.id] = self.validate_step("verit_lia_generic", step.cl, is_eval=is_eval)
                    else:
                        print(rule_name)
